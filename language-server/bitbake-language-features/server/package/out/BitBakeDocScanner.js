...
class BitBakeDocScanner {
    constructor() {
        ...
        this._docPath = path_1.default.join(__dirname, '../resources/docs');
        this._keywordInfo = KEYWORDS;
    }
    get bitbakeVariableInfo() {
        return this._bitbakeVariableInfo;
    }
    get yoctoVariableInfo() {
        return this._yoctoVariableInfo;
    }
    get variableFlagInfo() {
    ...
    parseBitbakeVariablesFile() {
        const variablesFilePath = path_1.default.join(this._docPath, 'bitbake-user-manual-ref-variables.rst');
        const variablesRegexForDoc = /^ {3}:term:`(?<name>[A-Z_]*?)`\n(?<definition>.*?)(?=^ {3}:term:|$(?!\n))/gsm;
        let file = '';
        try {
            file = fs_1.default.readFileSync(variablesFilePath, 'utf8');
        }
        catch {
            OutputLogger_1.logger.error(`Failed to read Bitbake variables at ${variablesFilePath}`);
        }
        const bitbakeVariableInfo = [];
    ...
    parseYoctoVariablesFile() {
        const filePath = path_1.default.join(this._docPath, 'variables.rst');
        ...
        try {
            ...
            OutputLogger_1.logger.error(`Failed to read Yocto variables at ${filePath}`);
        }
        const yoctoVariableInfo = [];
    ...
    parseYoctoTaskFile() {
        const yoctoTaskFilePath = path_1.default.join(this._docPath, 'tasks.rst');
        const yoctoTaskPattern = /(?<=``)((?<name>do_.*)``\n-*\n\n(?<description>(.*\n)*?))\n(?=(\.\. _ref)|((.*)\n(=+)))/g;
        let file = '';
        try {
            file = fs_1.default.readFileSync(yoctoTaskFilePath, 'utf8');
        }
        catch {
            OutputLogger_1.logger.warn(`Failed to read Yocto task file at ${yoctoTaskFilePath}`);
        }
        const yoctoTaskInfo = [];
    ...
    parseVariableFlagFile() {
        const variableFlagFilePath = path_1.default.join(this._docPath, 'bitbake-user-manual-metadata.rst');
        ...
        let file = '';
        try {
            file = fs_1.default.readFileSync(variableFlagFilePath, 'utf8');
        }
        catch {
            OutputLogger_1.logger.error(`Failed to read file at ${variableFlagFilePath}`);
        }
        const variableFlagSection = file.match(variableFlagSectionRegex);
        if (variableFlagSection === null) {
            OutputLogger_1.logger.warn(`No variable flag section found at ${variableFlagFilePath}. Is the regex correct?`);
            return;
        }
        const variableFlagInfo = [];
    ...
    parsePythonDatastoreFunction() {
        const filePath = path_1.default.join(this._docPath, 'bitbake-user-manual-metadata.rst');
        ...
        try {
            ...
            OutputLogger_1.logger.warn(`Failed to read file at ${filePath}`);
        }
        const pythonDatastoreFunction = [];
...
exports.BitBakeDocScanner = BitBakeDocScanner;
exports.bitBakeDocScanner = new BitBakeDocScanner();
//# sourceMappingURL=BitBakeDocScanner.js.map